...
export default function Home() {
  ...
  // Cargar menú, cuentas y ventas desde la API al iniciar
  useEffect(() => {
    fetch("/api/menu")
      .then((res) => res.json())
      .then(setMenu);
  ...
  // Calcular resumen por cuenta (sumando todos los items de ventas asociadas a cada cuenta)
  const resumen = accounts.map((cuenta) => {
    const total = ventas
    ...
    <div className='min-h-screen w-full flex flex-col items-center justify-center bg-gradient-to-br from-blue-100 via-white to-blue-200 py-8 px-2'>
      ...
      <div className='w-full flex flex-col md:flex-row md:items-start md:justify-center gap-8 max-w-5xl'>
        ...
        <div className='w-full md:w-2/3 flex flex-col items-center'>
          <div className='w-full max-w-xl bg-white/95 p-8 rounded-xl shadow-lg border border-blue-200'>
              ...
              <tbody>
                ...
                <tr className='border-t border-blue-300 font-bold'>
                  ...
                  <td className='py-2 text-right'>
                    ${resumen.reduce((sum, cuenta) => sum + cuenta.total, 0)}
                  </td>
                </tr>
              </tbody>
            </table>
